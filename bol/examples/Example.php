...
class Example {
    ...
    public static $bol_partner_site_id;
	...
	private static function getOffer($bRaw=0,$params='') {
	    // roduct request /catalog/v4/offers/{id} + queryParams
	    if(!isset($params['productid'])) $productid = '1002004010708531'; else $productid = urldecode($params['productid']);
    ...
    private static function getRelatedProducts($bRaw=0,$params='') {
    ...
    private static function getLists($bRaw=0,$params='') {
...
//        self::printValue(" ");
    ...
    private static function postBasket($bRaw=0) {
    ...
    private static function removeFromBasket($bRaw=0) {
    ...
    private static function postWishlist($bRaw=0) {
    ...
    private static function getWishlist($bRaw=0) {
        ...
        if(self::checkSession()) {
            ...
            if($bRaw!=0) {
                ...
                self::printValue('----');
                $i=1;
                if(isset($_SESSION['sessionusername'])) self::printValue('Wishlist from '.$_SESSION['sessionusername']);
    ...
    private static function deleteWishlist($bRaw=0) {
    ...
    private static function getAuthtokens($bRaw=0) {
        ...
        if(!isset($_SESSION['privatetoken'])) {
            ...
            if($bRaw!=0) {
                ...
                self::printValue('Mobile url: <a href="'.$authurl.'" target="_blank">'.$authurl.'</a> (advised to use)');
                self::printValue('Desktop url: <a href="'.$response->url.'" target="_blank">'.$response->url.'</a>');
    ...
    private static function getLogin($bRaw=0,$params='') {
        ...
        if(!isset($_SESSION['loggedin'])) {
            if(isset($_SESSION['privatetoken'])) {
                ...
                self::printValue('----');
                if(isset($_SESSION['created'])) self::$apiClient->setSessionId($_SESSION['sessionid']);
                ...
                self::printValue($response);
                self::printValue('----');
                self::printValue('By saving the private token you can get the data (if authorised) of the user at any time (until revoked by the user). To test you can copy this <a href="http://developers.bol.com/examples/phpexamplecode-v4/index.php?action=login&privatetoken='.$_SESSION['privatetoken'].'">link</a> in a new browser session.');
            } else {
    ...
    private static function setRefferer($bRaw=0) {
    ...
    private static function getReferrerFromPartnerUrl() {
        ...
        $url = 'http://partner.bol.com/click/click?p=1&t=url&s='.self::$bol_partner_site_id.'&f=API&url=http%3A%2F%2Fwww.bol.com%2Fnl%2F';
